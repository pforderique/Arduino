...
//you will eventually replace this starter step function with your improved step function!
...
void setup() {
  ...
  pos.x = 32; //start ball in center of screen!
  pos.y = 64; //start ball in center of screen!

  //apply initial force in random direction and magnitude:
  step(random(-EXCITEMENT,EXCITEMENT),random(-EXCITEMENT,EXCITEMENT)); 
...
void loop() {
  ...
  if (!digitalRead(BUTTON)){ //if pushed
  ...
  //pace the loop so that each time step is consistent:
  //note if we needed to do more things in our code/other than a simulation,...
  //...we'd likely execute the code above in an if statement
  //and not "block" the main loop with a while loop like this. For now this is fine
