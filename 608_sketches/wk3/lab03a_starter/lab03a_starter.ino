...
char ap_data[3000] = {0}; //global used for collecting data about access points 

const char NETWORK[] = "PCP-0672";
const char PASSWORD[] = "Limaperuu1";
...
int wifi_object_builder(char* object_string, uint32_t os_len, uint8_t channel, int signal_strength, uint8_t* mac_address) {
  int total_length = 5; // for the curly brackets and commas // if os_len does not include \0
  
  char macaddr[42];
  total_length += sprintf(macaddr, "\"macAddress\": \"%x:%x:%x:%x:%x:%x\"", 
                          mac_address[0], mac_address[1], mac_address[2],
                          mac_address[3], mac_address[4], mac_address[5]);
  char signalstrg[200];
  total_length += sprintf(signalstrg, "\"signalStrength\": %d", signal_strength);
  
  char age[200];
  total_length += sprintf(age, "\"age\": %d", 0);
  
  char chan[200];
  total_length += sprintf(chan, "\"channel\": %d", channel);
  
  // check that we have enough space!
  if(total_length > os_len) return 0;
  
  // Now format the whole json body!
  return sprintf(object_string, "{%s,%s,%s,%s}", macaddr, signalstrg, age, chan);
}
...
void loop() {
  ...
  if (!button_state && button_state != old_button_state) {
    ...
    if (n == 0) {
      ...
      Serial.println("-----------");
      //For Part Two of Lab03A, you should start working here. Create a staticJsonDoc of a reasonable size (few hundred bytes...)

      // getting the json part
      char *last_char = strrchr(response, '}');
      last_char[1] = '\0';
      char *first_char = strchr(response, '{');
      
      // DESERIALIZE THE JSON RESPONSE
      StaticJsonDocument<500> doc;
      DeserializationError error = deserializeJson(doc, first_char);
      // check for error
      if (error) {
        ...
        //update tft
        tft.fillScreen(TFT_BLACK); //fill background
        tft.setTextColor(TFT_GREEN, TFT_BLACK); //set color of font to hot pink foreground, black backgroun
        tft.setCursor(0, 1);
...
void send_to_608(double lat, double lng){
  char output[100];
  sprintf(request,"GET  http://608dev-2.net/sandbox/sc/jds99/lab03b/demo_loc.py?lat=%f&lon=%f HTTP/1.1\r\n", lat, lng);
