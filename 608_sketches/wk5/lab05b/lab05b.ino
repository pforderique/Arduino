...
void setup() {
  ...
  double diff = (2.0)/NOTE_COUNT;
  accel_thresholds[0] = -1.0;
  for(int i = 1; i < NOTE_COUNT + 1; i++){
    accel_thresholds[i] = accel_thresholds[i-1] + diff;
    Serial.println(accel_thresholds[i]);
  }
...
void run_instrument() {
  while(!digitalRead(BUTTON1)){
    ...
    for(int i = 1; i < NOTE_COUNT + 1; i++){
      if(y < accel_thresholds[i]){
        new_note = note_freqs[i - 1];
        break;
      }
    }
