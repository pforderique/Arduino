...
PWM_608::PWM_608(int op, float frequency, int invert_in) {
  ...
  invert = invert_in;
  on_amt = (int) (0.5*period);
}

void PWM_608::update() {
  //updates state of system based on millis() and duty cycle
  if(millis()%period < on_amt){
    if(invert == 0){digitalWrite(pin, HIGH);}
    else{digitalWrite(pin, LOW);}
  }else{
    if(invert == 0){digitalWrite(pin, LOW);}
...
void PWM_608::set_duty_cycle(float duty_cycle) {
  ...
  on_amt = (int)((duty_cycle/100)*period);
}
...
void loop() {
  if (Serial.available()) {
    ...
    // value = 0 - 100
    value = (int) (value/100.0*4095);
    if(backlight.invert == 1){
      ledcWrite(PWM_CHANNEL, 4096-1-value);
    }else{
